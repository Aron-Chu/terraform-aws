...
resource "aws_instance" "aron_node" {
  ...
  tags = {
    Name = "aron_node-${random_id.aron_node_id[count.index].dec}"
  }
...
resource "aws_lb_target_group_attachment" "aron_tg_attach" {
  count            = var.instance_count
  target_group_arn = var.lb_target_group_arn
  target_id        = aws_instance.aron_node[count.index].id
  port             = var.tg_port
}